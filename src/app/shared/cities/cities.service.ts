...
export class CitiesService implements ICitiesService {
  ...
  public async search(name: string): Promise<City> {
    const response = await fetch(
      `https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20geo.places(1)%20where%20text%3D%22${encodeURIComponent(
        name
      )}%22&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys`
    );
    const result = await response.json();
    const place = result.query.results.place;
    const locality = place.locality1;
  ...
  public async remove(id: number): Promise<City[]> {
    const index = this.cities.findIndex(city => city.woeid === id);
